...
- `help`, `doctor`를 제외한 모든 명령은 **target project root** 가 필요합니다.
- target project root 우선순위:
  1. `--project-root /path/to/service-app`
  2. 도구 저장소 루트의 `.openapi-projector.local.jsonc` 의 `projectRoot`
  3. legacy fallback: `.openapi-tool.local.jsonc` 의 `projectRoot`
- 위 설정이 모두 없으면 실행되지 않습니다.
...
그 다음 `.openapi-projector.local.jsonc` 에서 필수값만 채웁니다.
...
- `prepare`는 `init`이 필요하면 먼저 실행한 뒤 `refresh -> rules -> project`를 이어서 실행합니다.
...
세부 단계를 나눠서 실행하고 싶으면 기존 순서를 그대로 사용할 수 있습니다.
...
- 파일: `.openapi-projector.local.jsonc`
- 용도: target project root + `init` 기본값
- 예시 파일: `.openapi-projector.local.example.jsonc`
- legacy fallback: `.openapi-tool.local.jsonc`
...
- `.openapi-projector.local.jsonc` 기반 target project root 지정 지원
- `doctor`, `prepare` 명령으로 초기 사용 흐름 단순화
