...
openapi/review/project-rules/analysis.md와 analysis.json의 근거를 확인한 뒤, 규칙이 맞으면 openapi/config/project-rules.jsonc의 review.rulesReviewed를 true로 바꿔.
...
- `tsconfig.json` / `jsconfig.json` path alias 기반 helper import 경로 정규화
- 태그 폴더형 또는 flat endpoint 파일 배치
- 명시적 `2xx`/`2XX` 성공 응답이 있는 endpoint 생성
- 여러 request/response media type 중 지원 가능한 JSON 계열 또는 multipart request 선택
