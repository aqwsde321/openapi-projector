...
test(
  ...
  { concurrency: false },
  async () => {
    await withTempDir('openapi-projector-cli-', async (projectorWorkspace) => {
        ...
        await withToolLocalConfigs(
          ...
          async () => {
            ...
            const projectConfigSource = await readTextFile(
              projectConfigPath(projectorWorkspace),
            );

            assertMatchesAll(projectConfigSource, [
              /"sourceUrl": "https:\/\/projector\.example\.com\/v3\/api-docs"/,
            ]);
            await assertMissing(projectConfigPath(legacyWorkspace));
          },
        );
      });
    });
