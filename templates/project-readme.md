...
이 `openapi/` 디렉터리는 `openapi-projector init`으로 생성된 작업 공간입니다.
...
처음 적용할 때는 보통 규칙 검토 단계에서 멈춥니다. 이때 `openapi/review/project-rules/analysis.md`를 보고 `openapi/config/project-rules.jsonc`를 프로젝트에 맞게 수정한 뒤, `review.rulesReviewed`를 `true`로 바꾸고 `openapi-projector prepare`를 다시 실행합니다.
...
| --- | --- | --- | --- | --- |
| 1 | `openapi-projector doctor --check-url` | 설정과 OpenAPI JSON URL 접근 가능 여부를 확인합니다. | 없음 | `openapi/config/project.jsonc` |
| 2 | `openapi-projector refresh` | OpenAPI JSON을 내려받고 endpoint 문서, schema, 변경 요약을 생성합니다. | `openapi/review/changes/summary.md`, `openapi/review/docs/`, `openapi/review/generated/schema.ts` | `openapi/review/changes/summary.md` |
| 3 | `openapi-projector rules` | 현재 프로젝트의 API client/import/call style을 분석하고 규칙 초안을 만듭니다. | `openapi/review/project-rules/analysis.md`, `openapi/review/project-rules/analysis.json`, `openapi/config/project-rules.jsonc` | `openapi/review/project-rules/analysis.md`, `openapi/review/project-rules/analysis.json` |
...
| 5 | `openapi-projector project` | 검토된 규칙으로 DTO/API 후보 코드를 생성합니다. | `openapi/project/summary.md`, `openapi/project/src/openapi-generated/` | `openapi/project/summary.md` |
