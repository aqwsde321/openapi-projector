...
사람은 상단 요약과 AI에게 붙여넣을 프롬프트만 보면 됩니다. AI coding agent는 아래의 접힌 상세 지침까지 읽고 작업합니다.
...
1. `refresh`: OpenAPI JSON을 내려받고 이전 Swagger와 비교한 review 문서를 만듭니다.
...
`prepare` 실행 후 사람이 먼저 볼 파일:

| 파일 | 볼 내용 |
| --- | --- |
| `openapi/changes.md` | OpenAPI endpoint 추가/삭제/계약 변경 요약 |
| `openapi/changes.json` | 변경 요약의 machine-readable JSON |
...
| `openapi/project/summary.md` | `rulesReviewed=true` 이후 생성되는 DTO/API 후보 요약 |
...
| 2 | `npx --yes openapi-projector refresh` | OpenAPI JSON을 내려받고 이전 Swagger와 비교한 변경 문서를 생성합니다. | `openapi/changes.md`, `openapi/review/docs/`, `openapi/review/generated/schema.ts` | `openapi/changes.md` |
...
Read this workflow before modifying generated API code. The first goal is to compare the current Swagger/OpenAPI contract with the previous snapshot and explain what changed. Only after that should you inspect generated candidates, adapt the project rules to the current frontend project, and copy useful DTO/API code into the real application source tree.
...
5. Do not commit `openapi/changes.md`, `openapi/changes.json`, or `openapi/review/changes/*` by default; they are regenerated review artifacts.
...
`changes.md`, `changes.json`, `review/`, and `project/` are reproducible outputs. Commit only the real app code that you intentionally adapt from the candidates.
