...
프론트엔드 프로젝트에서 사용하는 AI coding agent에게 아래 프롬프트를 붙여넣으세요.
...
처음 실행하면 보통 2번 `rules`까지 실행한 뒤 멈춥니다. `project`는 `openapi/config/project-rules.jsonc`의 `review.rulesReviewed`가 `true`일 때만 실행됩니다.
