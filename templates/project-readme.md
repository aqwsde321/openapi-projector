...
| `openapi/.gitignore` | 재생성 산출물인 `_internal/`, `review/`, `project/`를 기본적으로 제외합니다. |

### 3. 사람이 먼저 prepare 실행하기 (선택)
...
| 파일 | 볼 내용 |
| --- | --- |
| `openapi/review/changes/summary.md` | OpenAPI endpoint 추가/삭제/계약 변경 요약 |
...
3. 사람이 npx --yes openapi-projector prepare를 미리 실행했다면 openapi/review/changes/summary.md와 openapi/review/project-rules/analysis.md를 먼저 확인해.
...
6. npx --yes openapi-projector refresh를 실행하고 openapi/review/changes/summary.md를 확인해.
