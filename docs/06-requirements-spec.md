...
- 시스템은 후보 산출물을 사람이거나 AI가 적용할 수 있도록 경로 정보와 파일 목록을 제공해야 합니다.
- manifest와 summary는 반영 판단 근거로 사용할 수 있어야 합니다.

### FR-E2. review-first 원칙

- 시스템은 기본적으로 `project` 단계에서 충분한 검토가 가능하도록 워크플로우를 유지해야 합니다.
...
- 사람이거나 AI가 후보 코드를 실제 반영 경로에 적용할 수 있을 만큼 정보가 제공된다.
...
- 사람이 `project` 결과를 검토하고 수정할 수 있는가
