...
- 현재 checkout CLI로 `init -> refresh -> generate -> rules -> prepare -> update -> doctor --check-url` 흐름 확인
...
릴리즈 smoke test도 repo가 아닌 빈 temp workspace에서 npm 배포본으로 다시 실행합니다.

```bash
pnpm run smoke:release -- --package openapi-projector@latest
```
