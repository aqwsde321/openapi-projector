...
- `Contract Changed`: request body, response body, parameter처럼 프론트 코드에 영향을 줄 수 있는 계약 변경
- `Doc Changed`: summary, description, tag처럼 계약 외 문서성 변경

`Added`, `Contract Changed`, `Doc Changed` 항목은 `project-rules.jsonc` 기준으로 생성될 DTO/API 후보 파일 링크도 함께 표시합니다. React Query hook 생성을 켠 경우 hook 후보도 같은 generated 디렉터리에 생깁니다. 링크 대상은 `project` 실행 후 생기는 `openapi/project/src/openapi-generated/...` 아래 파일입니다.

`Contract Changed`는 이전 catalog와 현재 catalog 모두 비교용 snapshot을 가지고 있으면 요청/응답 전체를 AS-IS / TO-BE 2열 Markdown 표로 표시합니다. 예를 들어 query parameter 추가, request body required 변경, response body field type 변경 같은 내용은 해당 줄에 추가/변경/삭제 표시가 붙어 `openapi/changes.md`와 `history/*.md`에 남습니다.
`Added`, `Contract Changed`, `Doc Changed` 항목은 operationId가 있으면 `sourceUrl` 또는 `swaggerUiUrl` 기준 Swagger UI 링크도 붙입니다. `Removed`는 삭제된 endpoint라 링크를 붙이지 않습니다. preview와 링크는 Markdown 전용 표시이며, `changes.json`은 자동화가 읽기 쉽도록 기존 요약 구조를 유지합니다.
